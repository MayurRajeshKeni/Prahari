...
def extract_features(df: pd.DataFrame, log_type: str) -> pd.DataFrame:
    ...
    if log_type == 'apache_access':
        ...
        df['is_static'] = df['url_path'].str.contains(r'\.(?:css|js|png|jpg|jpeg|gif|ico|woff|svg)$', case=False, regex=True, na=False).astype(int)
...
def process_logs(input_path: str, output_path: str, log_type: str, chunk_size: int = CHUNK_SIZE) -> None:
    ...
    if all_chunks:
        ...
        # Drop columns with zero variance across the aggregated dataset
        low_variance_cols = [col for col in final_df.columns if final_df[col].nunique() <= 1]
        if low_variance_cols:
            logger.info(f"Dropping low-variance columns: {low_variance_cols}")
            final_df = final_df.drop(columns=low_variance_cols)
